...
public class GraphFileWriter {
     ...
     *
     * @param graphLayoutEngine the graph layout engine to use in place of the SFDP default layout
     *                          engine
    ...
    /**
     * Overrides the default destination directory (system temp. directory) with the given
     * directory.
     *
     ...
     * @throws IOException if the specified destination directory couldn't be created or already
     *                     exists as a file
     */
    ...
    /**
     * Writes a directed, weighted graph to an SVG file in the destination directory with a randomly
     * generated filename.
     *
     * <br />
     *
     * This is done by first writing a DOT file with attributes specified by the class's
     * GraphDataToDOTExporterMapper.  Then the DOT file is converted to an SVG file via a command
     * line call to 'dot.exe' in the ZGRViewer directory.  (The DOT files are converted to SVG for
     * better visualization algrothims for larger data sets).
     *
     ...
     * @throws IOException if there is trouble writing the DOT file, reading the DOT file or writing
     *                     the SVG file.
     */
